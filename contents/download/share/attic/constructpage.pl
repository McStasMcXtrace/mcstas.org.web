...
$pwd="/home/mcstas/Builds/checkout/web/contents/download/share/attic";
opendir(DIR, $pwd);
@files = readdir(DIR);
@files = grep(!/constructpage.pl|^share|.md5|.html|name|CVS|hide|^\./,@files);

closedir(DIR);

$url = "/download/share/attic";

system("cat attic.html.top");
filetable($url,"Obsolete, perhaps still useful files previously in share",@files);
print "<hr>\n";
print "</body>\n";
print "</html>\n";
